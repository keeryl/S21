...
START_TEST(inverse_1) {
  ...
  s21_create_matrix(3, 3, &etalon_res);
  etalon_res.matrix[0][0] = 1;
  etalon_res.matrix[0][1] = -1;
  etalon_res.matrix[0][2] = 1;
  etalon_res.matrix[1][0] = -38;
  etalon_res.matrix[1][1] = 41;
  etalon_res.matrix[1][2] = -34;
  etalon_res.matrix[2][0] = 27;
  etalon_res.matrix[2][1] = -29;
  etalon_res.matrix[2][2] = 24;
  matrix_t res = {0};
  int status_code = s21_inverse_matrix(&A, &res);
  for (int i = 0; i < etalon_res.rows; i++)
    for (int j = 0; j < etalon_res.columns; j++)
      ck_assert_double_eq(res.matrix[i][j], etalon_res.matrix[i][j]);
  ck_assert_int_eq(status_code, OK);
